...
function hypertyLoaded(result) {
    ...
    initListeners();
    $.getScript("javascripts/reThinkCall/adapter.js");
...
function addBtnListener() {
    ...
    $('#settings').on('submit', function toggleSettings(event) {
...
function proceedCall(uid, dom) {
    // Patch as for the moment we don't use the IdP
    var index = uid.indexOf("://");
    var userEmail = uid;
    var userDomain = dom;
    if (index > -1) {
        userEmail = uid.slice(index + 3);
    }
    index = userEmail.indexOf("/");
    if (index > -1) {
        userEmail = userEmail.slice(index + 1);
    }
    hypertyWebRTC.search.users([uid], [userDomain], ['connection'], ['audio', 'video']).then((result) => {
